// Cloudflare Pages Function: RAG-based answer generation (v2)
// File path: functions/ask.js
// URL: /ask?q=<query>
//
// v2 changes:
// - Removed self-fetch (Airtable API called directly)
// - Multi-keyword OR matching with score-based ranking
// - Better error logging in flags
...
export async function onRequest(context) {
  ...
  try {
    ...
    const supportedCategories = {
      "omega3": ["\uC624\uBA54\uAC00", "omega", "epa", "dha", "ala", "dpa", "rtg", "\uC54C\uD2F0\uC9C0", "\uC5B4\uC720", "fish oil", "\uD06C\uB9B4", "ee", "tg"],
    ...
    // ─── [2] DIRECT AIRTABLE SEARCH (no self-fetch) ──
    const queryTokens = query.split(/\s+/).filter(function(t) { return t.length > 0; });
    const lowerTokens = queryTokens.map(function(t) { return t.toLowerCase(); });
    ...
    const knowledgeMatched = (knowledgeData.records || [])
      .map(function(record) {
        ...
        for (let i = 0; i < lowerTokens.length; i++) {
          if (haystack.indexOf(lowerTokens[i]) !== -1) {
            score++;
          }
        }
        return { record: record, score: score, fields: f };
      })
    ...
    if (knowledgeMatched.length > 0) {
      ...
      knowledgeMatched.forEach(function(item, idx) {
    ...
    if (faqMatched.length > 0) {
      ...
      faqMatched.forEach(function(item, idx) {
    ...
    // ─── [6] POST-PROCESS ────────────────────────────
    const sources = [];
    knowledgeMatched.forEach(function(k) {
      sources.push({ type: "knowledge", id: k.id, evidence: k.evidence || null });
    });
    faqMatched.forEach(function(f) {
