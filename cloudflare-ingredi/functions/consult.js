...
//
// Pipeline (parallel):
// 1. /ask → RAG 답변 (Claude Sonnet 4.6)
// 2. /match-profile → 프로필 자동 매칭
// 3. /recommend → 매칭 프로필로 Top 3 추천
// → 통합 응답
...
export async function onRequest(context) {
  ...
  // ─── PROFILE MATCHING (keyword first) ───────────
  ...
  try {
    ...
    const supportedCategories = {
      "omega3": ["\uC624\uBA54\uAC00", "omega", "epa", "dha", "ala", "dpa", "rtg", "\uC54C\uD2F0\uC9C0", "\uC5B4\uC720", "fish oil", "\uD06C\uB9B4", "ee", "tg", "\uD601\ud589", "\uD601\uC911", "\uC9C0\ubc29", "\uC2EC\ud608\uad00", "\uC5fc\uc99d", "\ub1cc", "\uc2DC\ub825"],
    ...
    // 카테고리에 직접적인 키워드는 없지만 일반 건강 질문일 수 있음 (혈관, 콜레스테롤 등)
    const healthKeywords = ["\uD658\ud589", "\uD58C\uc911", "\uc9c0\ubc29", "\uc5fc\uc99d", "\uc2EC\ud608\uad00", "\ub1cc", "\uc2DC\ub825", "\uace0\ud608\uc555", "\ub2f9\ub1a8", "\ucf5c\ub808\uC2A4\ud14c\ub864", "\uc5fc\uc99d", "\uad00\uc808"];
    ...
    if (!matchedCategory) matchedCategory = "omega3"; // 건강 키워드면 기본 오메가3

    // ─── [2] PARALLEL: knowledge + FAQ search ───
    ...
    const knowledgeMatched = (kRes.records || [])
      .map(r => {
        ...
        let score = 0;
        for (const t of lowerTokens) if (haystack.indexOf(t) !== -1) score++;
    ...
    // FAQ 매칭
    const fF_id = "FAQ_ID";
    const fF_q = "\uC9C8\uBB38(\uC0AC\uC6A9\uC790 \uD45C\uD604)";
    const fF_a = "\uB2F5\uBCC0(3\uC6D0\uCE59 \uC801\uC6A9)";
    ...
    if (knowledgeMatched.length === 0 && faqMatched.length === 0) {
      ...
      if (claudeResponse.ok) {
        ...
        answer = "\u26a0\ufe0f AI \ub2f5\ubcc0\uc744 \ubd88\ub7ec\uc62c \uc218 \uc5c6\uc5b4\uc694. \ub2e4\uc74c \uc81c\ud488 \ucd94\ucc9c\uc744 \ud655\uc778\ud574\uc8fc\uc138\uc694.";
      }
    }

    // ─── [6] RECOMMEND PRODUCTS (using matched profile) ──
    ...
    let filtered = scored.filter(item => {
      ...
      if (profile.filters && profile.filters.veganOnly) {
        ...
        const isVegan = nameLower.indexOf("\uC2DD\uBB3C\uC131") !== -1 || nameLower.indexOf("vegan") !== -1 || nameLower.indexOf("algae") !== -1;
