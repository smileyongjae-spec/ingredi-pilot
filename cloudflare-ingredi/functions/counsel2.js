// functions/counsel2.js  [v15.26 — LLM층 자체점검(350문항) 반영]
...
- JSON 문자열 값 안에서 큰따옴표(")를 쓰지 마세요. 인용이 필요하면 홑따옴표(\'')나 낫표(「」)를 쓰세요. 큰따옴표를 쓰면 응답 전체가 깨집니다.`;
    ...
    const ALCOHOL_RISK_RE = /해장술|아침.{0,8}(술|마시)|블랙\s*아웃|필름.{0,6}끊|술\s*없인|술\s*안\s*마시면.{0,10}(떨|잠|불안)|손이?\s*떨리|금주\s*실패|알코올\s*중독|알콜\s*중독|매일\s*(소주|맥주|위스키|와인).{0,8}(두|둘|2|세|3|반)\s*(병|캔)|낮술이?\s*늘|끊[을고]\s*수가?\s*없|못\s*끊/;
