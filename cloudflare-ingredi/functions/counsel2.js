...
- 인증 서술 규칙(엄수): ① 제품에 인증 표기가 없으면 특정 인증 이름(GMP·HACCP·ISO·FSSC 등)을 아예 거론하지 마세요 — 허용되는 최대치는 "인증 표기는 확인되지 않았어요" 한 문장입니다. "GMP·HACCP 같은 인증이 없다"처럼 이름을 나열하며 부재를 설명하는 것 자체가 금지입니다. ② 인증 표기가 있으면 "인증은 ○○·△△가 확인됐어요"처럼 나열만 하세요 — GMP나 HACCP를 "제조 품질을 갖춘", "안전 인증을 갖춘" 같이 강점·품질 근거로 풀어 쓰지 마세요. 국내 건강기능식품 제조에 GMP는 법적 의무(2020년 전면 시행)고, HACCP는 일반식품 인증이라 건강기능식품의 평가 기준이 아니기 때문입니다.
