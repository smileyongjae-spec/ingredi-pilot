...
//     상한은 max_tokens와 "항목당 3문장" 지시로만 두고, 자르더라도 문단 경계에서만(18문장 안전선).
//   - 줄바꿈은 모델 지시로 안 들어왔다 → 코드가 "첫째,/둘째,/셋째,/…/마지막으로," 앞에 빈 줄을 넣는다.
//   - 항목 뒤 마무리 한 문장(어느 축이 가장 큰 차이를 만드는지)을 요구해 설명이 카드 유도로 끝나지 않게 한다.
...
//     (body에 \n\n), 각 항목은 기준 숫자·이유·미표기 처리까지 2~3문장으로 쓰라고 지시.
//   - 문장 상한 절단이 join(" ")으로 줄바꿈을 지우던 문제 수리 — 원문 위치 기준으로 자르고 줄바꿈 보존.
//   - 표시는 consult.html이 body의 줄바꿈을 살려야 한다(별도 확인).
...
//     섭취 가능 여부를 묻는 질문에는 "드셔도 됩니다" 유지.
//   - 부정 평결: "솔직히 말씀드리면, 권하지 않아요" → "이 제품보다 나은 선택이 있어요" + 바로 다음 문장에 이유.
//     이유 없이 대안으로 넘어가면 수수료 유도로 읽힐 수 있어 이유 문장을 필수로 둔다.
//   - 인증 칸의 "-"·"없음" 등 무표기 값을 인증명으로 읽어 "인증은 -가 표기되어 있어요"가 나가던 문제(실측) 수리.
...
//     기존 M 게이트(치료 목적·약 대체·이상반응)를 빠져나갔다. 오메가3(항응고)·밀크씨슬(CYP 대사)·
//     비타민C(철분 흡수)는 실제 상호작용 질문이 들어오는 성분이라 안전 사안으로 코드 즉답한다.
//   - 판단을 모델에 맡기지 않는다: 약 이름을 아는 척하거나 "괜찮다"고 답하는 경로를 원천 차단.
//   - F7 보강: DB에 없는 임상 근거를 물으면 없다고만 하지 않고 확인처(식품안전나라·PubMed)를 알려준다.
...
//     [제품 데이터]의 인증 필드로 정본 문장을 붙인다: 표기 있음 → "인증은 ○○·△△가 표기되어 있어요."
//     / 없음 → "인증 표기는 확인되지 않았어요." — GMP 유무가 모델 편차로 뒤집히던 사고를 구조적으로 차단.
//   - 규칙 근거를 해외직구까지 확장: 미국도 FDA cGMP(21 CFR 111, 2010~ 전면 의무)라 "직구니까 GMP 강점" 우회 차단.
// functions/counsel2.js  v17.18  (2026-09-20)
// [v17.18 — 인증 서술 규칙 강화 (실측: v17.14 규칙을 양방향으로 회피)]
//   - 드시모네(무인증): "GMP·HACCP·FSSC22000 같은 인증이 확인되지 않고" — 부재 감점 서술 재발.
//   - 락티젠(유인증): "HACCP 안전 인증을 갖춘 제품" — 건기식 무관 인증을 강점으로 서술(반대 방향).
//   - 교체: 무표기 제품엔 특정 인증명 거론 자체 금지(한 문장 상한), 유표기 제품엔 나열만 허용.
...
// [v15.30 — 근거 용량 출처 표기: 식약처 → 복지부 → 연구 우선순위]
// [v15.29 — 축 점수 규칙 코드화 연결(_lib/axis-scores.js)]
//   - v15.29: scoresOf가 점수 컬럼 부재 시 코드 규칙으로 폴백(오메가3).
// [v15.28 — 제품 DB 교체 대응: 테이블명 중앙 설정(_lib/tables.js) + 이중가 비용 폴백]
//   - v15.28: 09.11 DB 교체. 테이블명은 TABLES에서, 1일비용은 쿠팡기준 → 옛 구조 → 네이버기준 순 폴백.
//   - v15.27: ALCOHOL_RISK_RE 확장 — "맥주 네 캔"(수량 4·5), "몰래 마시다 걸렸다",
//             "술 때문에 회사 잘릴 뻔" 등 재검에서 새 어휘로 빠져나간 3건의 패턴을 커버.
//   - v15.26: ① [카테고리 미확정] 플래그 — 미확정 상태에서 화자가 오메가3 기준으로 단정하던 물듦 차단
//                (등급질문 12건·가격/원료사 저점의 공통 원인). ② 알코올 의존 신호 감지(ALCOHOL_RISK_RE)
//                → 프롬프트 플래그 주입(감지는 코드, 발화는 프롬프트). ③ 밀크씨슬 화법: 음주 의존 대응·원료사 절제 보강.
//   - v15.25: ① PRODUCT_HINTS 밀크씨슬 정규식에 간 보호/걱정/챙/영양제/보조제·약주·반주·해장 추가
//                (자체점검 실패 2건 — "회식이 잦은데 간 보호되는 거"→눈, "약주를 즐기셔서 간 영양제"→오메가3).
//             ② explicitMultiCats를 debug meta에 노출 — 복수 카테고리 되묻기 플래그의 관측 가능성.
...
async function handle(context, sink) {
  ...
  // ─── 상수 (v7 계승) ──────────────────────────────
  const FAQ_TABLE = TABLES["FAQ"];
  const KNOW_TABLE = TABLES["knowledge"];
  ...
  const PRODUCT_HINTS = [
    ...
    // [v15.25] 커버 확장: 자체점검에서 "간 보호되는 거"·"간 영양제"가 힌트를 빠져나가 검색 투표로
    // 눈·오메가3에 오라우팅됨("간"은 1글자라 투표 토큰에서도 증발 — 주변 단어가 투표를 가로챈다).
    { cat: "milkthistle", re: /간\s*(이|은|을|도|에|수치|건강|기능|보호|걱정|챙|생각|영양제|보조제)|간\s*(안\s*좋|나빠)|간에\s*좋|숙취|음주|약주|반주|해장|술\s*(을|자주|많이|마시)|알코올/ }
  ];
  const DOMAIN_HINTS = [
  ...
  // [v17.20] 약물 병용 질의 — ①복용 중인 약의 존재 ②병용 표현이 함께 있을 때 발동.
  //   약 이름을 열거해 맞히려 하지 않는다(신약·상품명은 끝이 없음). "약"이라는 범주어와
  //   대표적 상호작용 약물군, 그리고 "~약 먹는데"류 표현을 함께 본다.
  const DRUG_NAME_RE = /와파린|아스피린|항응고|혈전|쿠마딘|헤파린|항혈소판|클로피도그렐|스타틴|리피토|크레스토|메트포르민|당뇨약|혈압약|고혈압약|갑상선|씬지로이드|levothyroxine|항생제|항우울|항경련|면역억제|스테로이드|피임약|호르몬제|처방약|먹는\s*약|복용\s*(중인|하는)\s*약|약을?\s*(먹|복용)/i;
  const COMBO_RE = /같이|함께|병용|동시에|겸해|같은\s*시간|괜찮|상호\s*작용|상호작용|충돌|문제\s*없|드셔도|먹어도|복용해도/;
  ...
  // [v17.1] 운영 주체·연락처·수익 구조 질문. "누가 만들었나" 계열, 연락 수단, 돈 버는 방식, 회사 여부, 신뢰 근거까지.
const ABOUT_QUERY = /(누가|어디서|어떤\s*회사|어느\s*회사|어디에서)\s*(만들|개발|운영|서비스)|만든\s*(사람|이|분|팀|주체)|제작자|개발자(가|는|\s*누구)|운영자|운영\s*주체|운영\s*(하는|주)체|회사(예요|에요|인가|입니까|야|냐|인가요)|법인|사업자|스타트업이|개인\s*(프로젝트|개발)|연락처|연락\s*(방법|수단|처)|문의(는|\s*방법|\s*어디|하려면|\s*할)|이메일(\s*(주소|알려|있))|메일\s*주소|고객\s*센터|고객센터|공식\s*(채널|계정|사이트)|제보|제휴|협업\s*문의|광고\s*(받|하나|해요|문의)|돈은?\s*(어떻게|어디서)|수익\s*(구조|모델|어떻게)|수수료(는|\s*받|\s*어떻게|\s*얼마)|어떻게\s*(돈을?\s*)?(버|벌)|무료(인가|예요|에요|야|인가요)|공짜|누가\s*돈|후원|스폰서|협찬(\s*받|은|을)?|about|소개\s*페이지/i;
const META_QUERY = /프롬프트|시스템\s*지시|이전\s*지시|무시하고|너\s*(는|누구|어떤|뭐|ai|에이아이|약사|의사|영양사|전문가|사람|로봇|봇|진짜)|\bai\s*(야|냐|니)\b|무슨\s*모델|모델이(야|니|에요)|당신은\s*누구|이\s*(서비스|사이트|앱)\s*(가|는|이)?\s*뭐|ingredi|인그레디|잉그레디|누가\s*만들|챗\s*봇|챗봇|무슨\s*기준으로\s*(판단|평가)|왜\s*(너를|널)\s*믿|jailbreak|ignore\s+previous/i;
  // [v15.19] 거래 문의와 신뢰 문의를 분리. 기존에는 하나의 정규식이 둘을 다 잡고 응답은 거래용
  //   하나뿐이라, "쿠팡 링크면 수수료 받는 거잖아요"에 "주문·배송·환불은 판매처에서"가 나갔다.
  //   또 배달(→"배달음식"), 약국(→"약국에서는 다른 걸 권하던데"), 세일·할인·최저가·직구·
  //   "가격이 왜"(→제품·가격 판단 질문)까지 잡아 정상 상담을 거래 안내로 튕겼다.
  const SERVICE_QUERY = /환불|반품|교환|배송|결제|쿠폰|주문|취소|고객센터|회원\s*가입|로그인|탈퇴|재입고|품절|약국\s*(에서\s*)?(파|팔|살|사|구매|판매|가면|가서|전용)|직구\s*(어디|사이트|방법|하는|대행)|최저가\s*(어디|사이트|찾|검색)/;
  const TRUST_QUERY = /광고\s*(받|비|료|협찬|수익|아니야|아니에요|아냐|냐|인가|맞죠|이지)|협찬|수수료|제휴|리베이트|뒷돈|돈\s*(을)?\s*(벌|버는|받고)|수익\s*(구조|모델|은|는)|어떻게\s*운영|순위\s*(를)?\s*(올려|조작|사)|업체한테/;
  // 급성 이상 반응·사고 — 검색 없이 즉시 M
  // v15.2: 부종·가려움(알레르기 신호) 추가 — "얼굴이 부었어요 가려워요"류 미커버 수정.
  // [v15.15] 영아(신생아~돌 전) 대상 질의 → M(소아과 이관). 이 시기 보충은 소아과 영역이고
  //   ingredi 제품 데이터도 이 월령 기준이 아니다. "신생아 엄마인데 유산균"(엄마 본인=모성 흐름)과
  //   구분하기 위해 MATERNAL_SELF면 발동하지 않는다. "6개월분"류 수량 표현 오탐을 막으려고
  //   개월 수는 아기 단어와 함께 나올 때만 잡는다.
  const INFANT_RE = /신생아|갓난|백일\s*(?:아기|된|아이)|돌\s*(?:안\s*된|전)|(?:아기|애기|아이|영아)\s*(?:가\s*)?(?:[0-9]|1[0-2])\s*개월|(?:[0-9]|1[0-2])\s*개월\s*(?:된\s*)?(?:아기|애기|아이|영아)/;
  const INFANT_MATERNAL_SELF = /(?:신생아|아기|애기|아이)\s*엄마|엄마\s*본인|수유|모유|임신/;
  ...
  const QUALITY_CFG = {
    ...
    milkthistle: { table: TABLES["밀크씨슬"],       anchor: 1,    primaryFields: ["실리마린_mg"],              addFields: [],            primaryLabel: "실리마린",    unit: "mg", anchorLabel: null,
  ...
  try {
    ...
    if (ABOUT_QUERY.test(query)) {
      ...
      const body = moneyFirst
        ? "이 서비스는 쿠팡 파트너스 활동의 일환으로, 쿠팡 구매 링크를 통하는 경우 일정액의 수수료를 받을 수 있습니다. 수수료는 순위·등급·추천에 영향을 주지 않으며, 광고비나 협찬은 받지 않습니다. 단, 쿠팡에 제품이 없어서 네이버 등 다른 e커머스로 링크되는 경우에는 수수료와 상관이 없습니다.\n\n· 소개 페이지: ingredi.kr/about.html"
        : isContact
    ...
    // [v15.19] 신뢰·수익모델 문의 — 광고 없는 서비스에 가장 중요한 질문이므로 코드가 사실을 확정한다.
    //   (모델이 지어내면 안 되는 사업 사실: 수수료는 판매가 비례, 순위 매수 경로 없음, 다른 수익원 없음.)
    //   이해충돌을 먼저 인정하고 안전장치를 보여주는 순서 — "저희는 공정합니다"는 역효과다.
    if (TRUST_QUERY.test(query)) {
      return respond(fixedPayload("X",
        // [v17.5] 확정 문안 — 군더더기 없이 사실만. 자세한 내용은 소개 페이지로 보낸다.
        "이 서비스는 쿠팡 파트너스 활동의 일환으로, 쿠팡 구매 링크를 통하는 경우 일정액의 수수료를 받을 수 있습니다. 수수료는 순위·등급·추천에 영향을 주지 않으며, 광고비나 협찬은 받지 않습니다. 단, 쿠팡에 제품이 없어서 네이버 등 다른 e커머스로 링크되는 경우에는 수수료와 상관이 없습니다.\n\n· 소개 페이지: ingredi.kr/about.html",
        { chips: ["등급 기준이 궁금해요", "오메가3 보러가기", "운동 보충제 보러가기"],
          chips_prompts: ["등급은 어떤 기준으로 정해요?", "go:/app.html?category=오메가3", "go:/sports.html"] }
    ...
    // [v15.16] 응급 문구를 무조건형으로 교체. 기존 문구는 "증상이 심하면 바로 병원으로"처럼
    //   응급에 조건절을 달았고, 과다복용에도 "복용 중인 제품이 있다면 중단하시고"라는 어긋난
    //   안내를 했다. 1,000문항 점검에서 이 게이트에 걸린 5건 중 4건이 실패(아나필락시스·의식저하
    //   ·아이 과량 포함) — 게이트를 통과해 LLM이 답한 쪽이 오히려 나았다. 조건절 제거 + 119 명시.
    if (ACUTE_EMERGENCY.test(query)) {
      return respond(fixedPayload("M",
        "지금 바로 119에 연락하시거나 응급실로 가세요. 영양제로 판단할 상황이 아니에요.",
        { handoff: "드시던 제품이 있으면 더 드시지 마시고, 그대로 가져가서 보여주세요. 성분 확인이 빨라집니다." }
      ), { gate: "emergency", demographics });
    }

    // [v15.15] 영아 게이트 — 어린이 게이트(만 3세~)보다 앞. 유산균이라도 예외 없이 M.
    if (INFANT_RE.test(query) && !INFANT_MATERNAL_SELF.test(query)) {
      return respond(fixedPayload("M",
        "돌 전 아기는 제가 판단드릴 수 있는 범위가 아니에요. 이 시기는 장내 환경이 자리 잡는 중이라, 유산균이라도 소아과에서 먼저 확인하시는 게 맞아요. 제가 보는 제품 데이터가 이 월령 기준이 아니라서 그대로 적용해 드릴 수가 없어요.",
        { handoff: "소아과에서 물어보세요: \"지금 월령에 프로바이오틱스를 시작해도 될까요? 시작한다면 어떤 제품이 적절한가요?\"" }
      ), { gate: "infant_scope", demographics });
    }

    // [v17.20] 약물 병용 게이트 — 약 + 병용 표현이면 판단하지 않고 의료 상담으로 넘긴다.
    // 다루는 5개 성분 중 오메가3·밀크씨슬·비타민C는 실제로 약물 상호작용이 보고되는 성분이라,
    // "괜찮다/안 괜찮다"를 우리가 답하는 것 자체가 위험하다. 대신 무엇을 물어볼지까지 준다.
    if (DRUG_NAME_RE.test(query) && COMBO_RE.test(query)) {
      return respond(fixedPayload("M",
        "약과 함께 드시는 건 제가 판단드릴 수 있는 부분이 아니에요. 같은 성분이라도 복용 중인 약과 용량에 따라 답이 달라져서, 처방한 의사나 약국에서 확인하셔야 정확합니다.",
        { handoff: "약사님께 이렇게 물어보세요: \"지금 복용 중인 약과 이 성분을 같이 먹어도 되는지, 시간 간격을 둬야 하는지 알려주세요.\" 제품 라벨이나 사진을 함께 보여주시면 확인이 빨라집니다." }
      ), { gate: "drug_interaction", demographics });
    }
    ...
    if (matchedCategory && QUALITY_CFG[matchedCategory] && (pRecords || []).length) {
      ...
      const items = (pRecords || []).map(r => {
        const f = r.fields || {};
        const { primary: rawPri } = rawPrimaryOf(f, cfg);
        const qx = qualityFromFields(matchedCategory, f);
        const q = qx ? qx.quality : null;
        // [v16.0] 눈: 주 표방 성분(루테인/아스타잔틴) 값·라벨을 화자에게 — 지아잔틴 합산 폐지
        const primary = (qx && qx.value != null && matchedCategory === "eye") ? qx.value : rawPri;
        return {
          product_id: getField(f, "product_id", "productId") || r.id,
          name: getField(f, "제품명", "네이버_제품명", "name") || "",
          primary_mg: primary,
          primary_label: qx && qx.label ? qx.label : null,
      ...
      if (productMatchRecord) {
        ...
        if (!seen.has(pid)) {
          ...
          if (hit) topProducts.push(hit);
            ...
            const { primary: rawPp } = rawPrimaryOf(pf, cfg);
            const pqx = qualityFromFields(matchedCategory, pf);
            const pq = pqx ? pqx.quality : null;
            const ppri = (pqx && pqx.value != null && matchedCategory === "eye") ? pqx.value : rawPp;
            topProducts.push({
...
이 근거 위에서 당신의 일은 정보 나열이 아니라 판단을 내려주는 것입니다. 팔아야 할 물건이 없어서 편하게 말합니다. 기준을 채운 제품에는 "기준을 잘 채운 제품이에요", 아닌 제품에는 "이 제품보다 나은 선택이 있어요"라고 분명히 말합니다. 아니라고 말할 수 있기 때문에 당신의 "괜찮아요"에 무게가 있습니다.
...
4) 정보가 충분하면 V (즉시 평결), 판단을 바꿀 핵심 정보 하나가 비어 있으면 Q (되묻기).
   ...
   - Q의 세 필드는 화면에서 [question(제목) → body(본문) → 칩 → default_answer(고르지 않으셔도 됩니다)] 순으로 나란히 보입니다. 역할이 겹치면 같은 질문을 두 번 하는 꼴이 됩니다:
     · question: 물음표로 끝나는 질문 한 문장만. 서술문·인사·설명을 넣지 마세요. (✗ "성분 우선으로 고르실지 알면 바로 알려드릴 수 있어요." → ✓ "성분과 가격 중 뭘 더 보시겠어요?")
     · body: 왜 그걸 묻는지에 대한 배경만. 질문을 다시 하지 마세요 — 물음표로 끝내지 마세요. question이 바로 위에서 이미 묻고 있습니다.
     · default_answer: 고르지 않아도 되는 완결된 답. "알려주시면 골라드릴게요"처럼 되묻거나 미루지 마세요. 반드시 제품이나 방향을 근거와 함께 제시합니다.
   - 사용자가 되묻기를 거부하면("그냥 골라줘", "묻지 말고", "빨리", "결론만", "알아서") Q를 쓰지 말고 V로 즉답하세요. 카테고리가 불명확하면 되묻지 말고 오메가3를 권하되, [제품 데이터]가 없으면 제품명을 지어내지 말고 근거(식약처 인정 기능·보충 필요성)로 이유를 대고 비교 페이지로 안내한 뒤, 칩으로 다른 카테고리를 고를 수 있게 하세요.
...
3. 부정 평결 3원칙: 사람이 아니라 제품·광고를 문제 삼는다 / "이 제품보다 나은 선택이 있어요"로 열고 바로 다음 문장에 이유(지적한 결함)를 밝힌다 — 이유 없이 대안으로 넘어가지 않는다 / 반드시 대안(alternatives)으로 끝낸다.
...
- A: positive. "기준을 잘 채운 제품이에요."
- B: positive + 한계 한 번. "기준을 잘 채운 제품이에요. 최고급은 아니지만 충분히 좋은 제품이에요."
...
- 원료사는 참고 정보로만 언급합니다. 원료사가 유명하다는 이유로 제품을 밀지 않으며, "원료사 좋은 걸로 추천해달라"는 요청에도 원산지·원료사를 근거로 특정 제품을 내세우지 않고 함량 표기·비용이라는 실제 기준으로 되돌립니다.
- 음주 문제 신호(매일 과음·해장술·블랙아웃·손떨림·금주 실패·"술 없인 못 산다")가 보이면 영양제 답변으로 끝내지 않습니다. 음주 자체가 본체임을 비난 없이 짚고, 전문 도움(병원·보건소 금주 클리닉)을 안내하며, 밀크씨슬이 음주 피해를 상쇄하지 못함을 분명히 합니다.
...
- [지식]·[FAQ]에 없는 임상 근거·연구 결과를 요구하면: 우리 DB에 등록된 근거까지만 말하고, 없는 연구는 지어내지 않습니다. 대신 확인처를 알려주세요 — 기능성·인정 여부는 식약처 식품안전나라, 원논문은 PubMed. "자료가 없다"에서 끝내지 않습니다.
...
- 근거 용량(앵커): 함량 점수의 만점 기준. 출처 우선순위는 ①식약처 일일섭취량 ②(없으면) 보건복지부 섭취기준 ③(둘 다 없으면) 유력 연구·학회 순입니다. 오메가3 EPA+DHA 1,000mg(식약처 500~2,000mg 중), 눈 루테인+지아잔틴 20mg(고시형 루테인 10~20mg·개별인정 복합추출물 합 10~20mg 상한)과 아스타잔틴 12mg(고시형 헤마토코쿠스추출물 4~12mg 상한), 유산균 보장균수 100억(식약처 1억~100억 상한), 비타민C 1,000mg(식약처 기준 없음 → 복지부 섭취기준 내 보충 목적 상한). 근거 용량을 말할 때는 출처(식약처/복지부/연구)를 함께 말하고, "임상 연구 용량"처럼 뭉뚱그리지 않습니다. 눈 20mg을 AREDS2 시험 용량이라고 말하지 않습니다(AREDS2는 루테인 10mg+지아잔틴 2mg).
...
- 시스템 내부 사정을 사용자에게 설명하지 마세요. "제품 데이터가 로드되지 않아서", "데이터가 ○○ 기준으로 불러와져 있어서", "이 목록에서는" 같은 말은 금지입니다 — 사용자는 목록도 로딩 상태도 보지 못합니다. 못 드리는 게 있으면 이유를 대지 말고 할 수 있는 것으로 안내하세요.
...
- JSON 문자열 값 안에서 큰따옴표(")를 쓰지 마세요. 인용이 필요하면 홑따옴표(\'')나 낫표(「」)를 쓰세요. 큰따옴표를 쓰면 응답 전체가 깨집니다.`;
    ...
    if (productContext.length) {
      ...
      const publicView = productContext.map(p => ({
        ...
        [mcfg ? mcfg.primaryLabel : "함량"]:
          p.primary_mg != null ? ((p.primary_label && p.primary_label !== (mcfg && mcfg.primaryLabel) ? p.primary_label + " " : "") + p.primary_mg.toLocaleString() + (mcfg ? mcfg.unit : "")) : null,
        ...(p.over_limit ? { 상한초과: "식약처·복지부 상한 초과 — 의사 상담 권고" } : {}),
        ...(p.hold_reason ? { 평가보류: p.hold_reason } : {}),
      ...
      productBlock += "\n근거 용량 앵커: " + (mcfg ? mcfg.anchorLabel : "카테고리별 근거 용량") + " (출처: " + ({ omega3:"식약처 일일섭취량", eye:"식약처 일일섭취량", probiotics:"식약처 일일섭취량", vitaminC:"보건복지부 섭취기준(식약처 기준 없음)" }[matchedCategory] || "카테고리 기준") + ").";
      ...
      productBlock += "\n근거 용량 앵커: 오메가3 EPA+DHA 1,000mg(식약처) / 루테인+지아잔틴 20mg(식약처) / 유산균 보장균수 100억(식약처) / 비타민C 1,000mg(복지부).";
      // [v15.26] 자체점검 결함: 미확정 상태에서 화자가 오메가3 기준(EPA·IFOS·등급)으로 단정 답변
      // ("품질 점수 몇 점?"→"오메가3 제품에 붙는 수치예요"). 검색 지식에 오메가3 문서가 딸려와도
      // 특정 카테고리를 전제하지 않게 못박는다.
      if (!matchedCategory) productBlock += "\n[카테고리 미확정] 어느 카테고리 질문인지 아직 모릅니다. 특정 카테고리(오메가3 등)의 기준·앵커·등급·점수를 전제로 단정하지 말고, 카테고리를 먼저 확인하세요. 등급·점수 질문이면 '카테고리마다 기준이 다르고 밀크씨슬은 등급을 매기지 않는다'까지 함께 안내하세요.";
    }
    ...
    let flagBlock = "";
    // [v15.26] 알코올 의존 신호 — 밀크씨슬 상담에 위험 음주가 실려 온다(자체점검 15문항 중 9건이
    // 영양제 답변만 하고 신호를 흘려보냄). 감지는 코드, 발화는 프롬프트 규칙(아래 주입)에 맡긴다.
    // [v15.27] 재검 잔여 3건 커버: "네 캔"류 수량 확장, 몰래 마심, 술로 인한 사회적 문제(직장·가정).
    const ALCOHOL_RISK_RE = /해장술|아침.{0,8}(술|마시)|블랙\s*아웃|필름.{0,6}끊|술\s*없인|술\s*안\s*마시면.{0,10}(떨|잠|불안)|손이?\s*떨리|금주\s*실패|알코올\s*중독|알콜\s*중독|매일\s*(소주|맥주|위스키|와인).{0,10}(두|둘|2|세|3|네|넷|4|다섯|5|반)\s*(병|캔)|(퇴근|저녁).{0,10}(맥주|소주).{0,8}(네|넷|4|다섯|5|세|3)\s*(캔|병)|몰래\s*(마시|마신|먹다|먹었)|술\s*(때문에|땜에).{0,12}(잘리|잘릴|혼나|싸우|이혼|문제|사고)|낮술이?\s*늘|끊[을고]\s*수가?\s*없|못\s*끊/;
    if (ALCOHOL_RISK_RE.test(userText)) {
      flagBlock += "\n\n[음주 위험 신호] 사용자 발화에 알코올 의존 가능성 신호가 있습니다. 영양제 추천으로만 답하지 마세요: ①음주 패턴 자체가 건강 문제의 본체임을 비난 없이 짚고 ②전문 도움(가까운 병원, 보건소 금주 클리닉, 알코올 상담)을 한 문장으로 안내하고 ③밀크씨슬은 음주 피해를 상쇄하지 못하며 '간 건강 도움' 기능뿐임을 명확히 하세요. 그 다음에야 제품 이야기를 짧게 합니다.";
    }
    if (productMatchRecord) {
    ...
    if (explainIntent) {
      flagBlock += `\n\n[설명 요청] 사용자는 제품이 아니라 판단 기준을 물었습니다. alternatives는 빈 배열로 두세요 — 설명을 묻는데 카드를 붙이면 판매 화면처럼 읽힙니다. body는 이렇게 씁니다: 첫 줄에 한 문장 요약 → 항목마다 "첫째, …" "둘째, …" "셋째, …"로 시작해 항목당 정확히 2~3문장(기준 숫자 → 왜 그 기준인지 → 표기가 없을 때 처리) → 마지막에 "실제로 제품 간 차이를 가장 크게 만드는 건 ○○이에요" 같은 마무리 한 문장. "세 가지" 같이 개수를 예고했으면 그 수만큼 빠짐없이 쓰고, 예고한 항목을 다 쓰기 전에 끝내지 마세요. 항목당 3문장을 넘기지 마세요 — 길어지면 뒤 항목이 잘립니다. 추천은 칩("성분 우선으로 추천해줘", "가성비 좋은 걸로 추천해줘")으로만 제안합니다.`;
    }
    ...
    const reqBody = JSON.stringify({
      model: MODEL, max_tokens: evalMode ? 450 : (explainIntent ? 1500 : 900),   // [v17.13/14] 출력 상한 — 평가 모드는 더 짧게 / [v17.22] 700→900(잘림 시 카드 소실 실측)
  ...
  function checkSpeakerRules(pl, cat) {
    ...
    // [v15.18] Q 중복 질문 — question이 있는데 body도 물음표로 끝남(화면에서 같은 질문 두 번).
    if (pl.policy === "Q" && pl.question && /\?\s*$/.test(String(pl.body || "").trim())) v.push("dup_question(3.9)");
  ...
  // ─── [10] JSON 계약 파싱 + 검증 ─────────────────
    ...
    if (!payload || !payload.policy || typeof payload.body !== "string") {
      ...
      if (!evalMode && payload.policy === "V" && typeof payload.body === "string") {
        const _cap = explainIntent ? 18 : 3;   // [v17.28] 설명 요청은 안전선만(18) — 실제 길이는 항목당 3문장 지시로
        // [v17.26] 줄바꿈을 보존하며 자른다 — join(" ")은 문단 구분을 지웠다.
        const _re = /[.!?](?=\s|$)/g; let _n = 0, _m, _cut = -1;
        while ((_m = _re.exec(payload.body))) { _n++; if (_n === _cap) { _cut = _m.index + 1; break; } }
        if (_cut > 0 && _cut < payload.body.trim().length) payload.body = payload.body.slice(0, _cut).trim();
      }
      // [v17.25] 설명 요청에는 제품 카드를 붙이지 않는다 — 추천은 칩을 눌러 받는다.
      if (explainIntent && !evalMode) { payload.alternatives = []; payload.alternatives_note = null; }
      // [v17.28] 문단 나누기는 코드가 한다 — 모델이 \n\n 지시를 지키지 않음(실측).
      if (explainIntent && typeof payload.body === "string" && payload.body) {
        payload.body = payload.body
          .replace(/\s*\n+\s*/g, "\n")
          .replace(/\s*(?=(?:첫째|둘째|셋째|넷째|다섯째|여섯째|마지막으로|끝으로|정리하면|결론적으로|실제로 제품 간)[,\s])/g, "\n\n")
          .replace(/\n{3,}/g, "\n\n").trim();
      }

      // [v17.19] 지목 평결의 인증 문장은 코드가 확정한다 — 모델이 쓴 인증 문장은 제거하고 정본을 붙인다.
      // (v17.14→v17.18 두 차례 지시 강화에도 "HACCP 안전 인증을 갖춘"/"GMP·HACCP가 확인되지 않고"류
      //  이탈이 반복 실측되어, 사실 문장을 지시가 아닌 코드로 고정한다.)
      if (productMatchRecord && payload.policy === "V" && typeof payload.body === "string" && payload.body) {
        const _certRaw = String(getField(productMatchRecord.fields || {}, "인증") || "").trim();
        const _certToks = _certRaw.split(/[,\/·]/).map(t => t.trim()).filter(t => t && !/^[-–—]+$|^(없음|해당\s*없음|미표기|무|n\/?a)$/i.test(t));   // [v17.24] "-" 등 무표기 값 제외
