...
export async function onRequest(context) {
  ...
  if (cfg.unscored) {
    // [v7.5] 미수집 레코드 숨김 — 파트너 수집 전 상태(이름+쿠팡 URL만 있고 함량·비용·가격·이미지
    // 전부 결측)는 목록에서 제외한다. 빈 카드 35개가 하단에 깔리면 첫인상을 해친다(시장 테스트 직전 결정).
    // 함량이든 가격이든 이미지든 하나라도 채워지면 자동 복귀하므로 수집 진행에 따라 저절로 늘어난다.
    const before = items.length;
    for (let i = items.length - 1; i >= 0; i--) {
      const it = items[i];
      const hasAny = it.primaryValue > 0 || num(it.extra && it.extra["밀크씨슬_mg"]) > 0 ||
                     it.dailyCost > 0 || it.price > 0 || !!it.image;
      if (!hasAny) items.splice(i, 1);
    }
