...
// [v1.7] 도핑 비유·부정 문맥 가드 — "HMB가 스테로이드급이라던데", "보충제는 다 스테로이드 같은 거
// 아니에요?"처럼 통념·비유 질문이 DOPING_RE("스테로이드" 단독)에 삼켜져 교정 기회가 차단되던 결함
// (LLM층 자체점검에서 통념교정 8건 FAIL의 주원인). 사용 의도 표현이 함께 있으면 가드를 무시하고 게이트 유지.
const DOPING_ANALOGY_RE = /스테로이드\s*(급|처럼|같|랑|이랑|아니|은\s*아니|수준|비슷)|약\s*같은\s*거\s*아니/;
const DOPING_INTENT_RE = /사이클|복용|추천|구매|구입|어디서|시작|주사|스택|맞\s*(을|고|으면)|파는|사는\s*법/;
function gateCheck(msg) {
  ...
  if (DOPING_RE.test(msg) && !(DOPING_ANALOGY_RE.test(msg) && !DOPING_INTENT_RE.test(msg))) {
    return {
      policy: "X",
  ...
  // [v1.7] 섭식 위험 행위 게이트 — 먹토·씹뱉·이뇨제/변비약/하제를 감량 수단으로 쓰는 신호.
  // 보충제 조합으로 답하면 위험을 승인하는 꼴이라 코드가 고정 응답한다. 완만한 절식 신호(한 끼·저열량)는
  // 프롬프트 규칙이 다룬다(맥락 판단 필요).
  if (/먹토|씹뱉|(이뇨제|변비약|하제|설사약).{0,14}(커팅|감량|빼|다이어트|체중)|(커팅|감량|다이어트).{0,14}(이뇨제|변비약|하제|설사약)/.test(msg)) {
    return {
      policy: "M",
      reply: "말씀하신 방법은 체중이 아니라 몸의 수분과 전해질을 깎는 방식이라, 보충제 조합으로 도와드릴 수 있는 영역이 아니에요. 이런 방식이 반복되고 있다면 혼자 조절하기 어려운 신호일 수 있어요. 병원(가정의학과·정신건강의학과)이나 국가 섭식장애 상담을 먼저 알아봐 주세요. 건강한 감량 페이스와 그때 쓸 수 있는 보충제는 언제든 도와드릴게요.",
      chips: []
    };
  }
...
[안전 대응 — 반드시 지킬 것]
- 극단 절식 신호(하루 한 끼, 800kcal 이하, 단식하며 운동, 극단 커팅): 보충제 조합을 답하기 전에 그 방식의 위험(근손실·요요·수행 저하)을 먼저 짚고, 지속 가능한 감량 페이스로 방향을 튼다. 절식을 전제로 받아들인 채 조합만 짜주지 않는다.
- 과량 섭취 + 이상 증상(심장 두근거림·손떨림·불면·어지럼): 첫 문장은 해당 제품 중단 안내다. 증상이 심하거나 지속되면 병원을 권한다. 용량 지식 설명은 그 다음이다.
- 임신·수유: 판단은 산부인과·약사에게 넘기되, 회피로 끝내지 않는다. 일반 정보(예: 단백질은 식품 유래 성분, 임신 중 단백질 필요량 자체는 증가)와 확인 포인트(감미료·카페인·첨가물 성분표를 의사에게 보여드리기)를 함께 제공해 다음 행동을 만들어 준다.
- 청소년 가능성 신호("학생", "입시", 나이 불명의 어린 말투): 카페인·부스터 질문이면 나이를 먼저 확인하고, 미성년이면 부스터를 권하지 않는 원칙(수면·식사·단백질 우선)을 말한다. "나이에 따라 금지는 아니지만" 같은 조건부 허용 화법을 쓰지 않는다.
- 스테로이드 비유·통념("스테로이드급이라던데", "보충제 다 스테로이드 아니냐"): 금지 약물 안내 거절 문구로 답하지 말고, 보충제(일반식품)와 약물의 차이를 근거로 교정한다. HMB·크레아틴 등 합법 성분을 약물로 오인하지 않는다.
...
- 베타알라닌: 1~4분 고강도 운동에 1일 3.2~6.4g을 4주 이상 누적 섭취 시 효과(ISSN 2015). 이 서비스의 등급 기준은 유효 범위 하한 3,200mg/일. 따끔거림(감각이상)은 무해하며 분할 섭취로 완화.
- 시트룰린: 1회 3g 이상(말레이트 6~8g 상당)에서 조건부 근거. 국내 "아르기닌·시트룰린" 제품은 대부분 L-아르기닌 6,000mg에 시트룰린 소량이라 등급을 매기지 않음.

[지식 — 근거등급 (이 서비스의 축)]
- ISSN 기준(지침 있음): 단백질 파우더·게이너, 크레아틴, EAA, 베타알라닌, 부스터(카페인·시트룰린·베타알라닌 표방 성분)
- ISSN 기준·조건부: HMB
- 기준 없음(지침 없음): BCAA 단독, 글루타민(근성장 목적), 카르니틴(지방감량 목적), 아르기닌·시트룰린(아르기닌 위주 제품)
- "기준 없음"은 나쁘다는 뜻이 아니라 "학회가 지침을 낼 만큼 근거가 쌓이지 않았다"는 사실 진술이다. 그렇게 말하라.

[지식 — 등급 산식 (사용자가 등급·점수를 물으면 이대로)]
- 단백질 파우더·게이너 = [1회 단백질 g ÷ 25g(게이너 40g) 충족률 × 0.5] + [원료 등급 × 0.3] + [인증 × 0.2]. 원료 등급은 라벨 첫 표기(주원료) 기준 WPI·WPH 100 / MPI(분리우유단백) 85 / ISP(분리대두) 75 / WPC·MPC·MCC(농축유청·농축우유·미셀라카제인) 60 / 미기재 30. 가수분해(WPH)는 소화 속도이지 순도가 아니라 WPI와 동급. 카제인 제품은 MPI 또는 MCC로 분류하며 "MCI" 같은 표에 없는 코드는 쓰지 않는다. 부스터 안의 베타알라닌 앵커는 1회 1,600mg(급성), 베타알라닌 단독 제품 앵커는 1일 3,200mg — 둘을 구분해 말한다.
- 크레아틴·EAA·HMB·베타알라닌·부스터 = [함량 충족률 × 0.7] + [원료 브랜드 × 0.1] + [인증 × 0.2]. 원료 브랜드는 등록 상표 원료(Creapure·CarnoSyn·myHMB·AjiPure 등) 100 / 원산지·제조사명 70 / 미기재 35.
- 부스터는 표방한 근거 성분(카페인 ≥ 체중×1.5mg, 시트룰린 ≥1,500mg, 베타알라닌 ≥800mg)마다 ISSN 앵커(카페인 체중×3mg·시트룰린 3,000·베타알라닌 1,600)로 재서 평균. 아르기닌·타우린만 든 제품은 근거 성분이 없어 등급 없음.
- 인증은 종류별 가산(상한 100): 도핑 검사 인증(Informed Sport·Informed Choice·NSF Certified for Sport) 40 / 3rd party tested 25 / GMP 20 / FSSC·ISO 22000 15 / HACCP 10. Non-GMO·Vegan·Kosher·Halal은 배합 속성이라 0.
- 미표기: 함량이 없으면 평가 보류. 원료 등급·브랜드가 비면 최하 점수의 절반으로 등급 부여. 인증란이 비면 0.
- 등급 컷 A 85 / B 70 / C 55 / D 40 — 절대 기준. 가격·리뷰는 등급에 없음.
- "기준 없음"은 나쁘다는 뜻이 아니라 "학회가 지침을 낼 만큼 근거가 쌓이지 않았다"는 사실 진술이다. 그렇게 말하라.
...
async function handle(context, headers) {
  ...
  if (request.method === "GET") {
    ...
    const diag = {
      version: "v1.7",   // [v1.7] LLM층 자체점검(647문항) 반영: 도핑 비유 가드·섭식위험 게이트·안전 프롬프트
      env: {
